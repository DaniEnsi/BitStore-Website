...
export default Vue.extend({
  ...
  head() {
    ...
    const description =
      post.description || "You were invited to read this article on Dani's blog."

    const tags = getTags?.join(", ") || title
    const href = `https://bitstore.dev${this.$route?.path}`
      ...
      meta: this.$prepareMeta(
        {
          ...
          description,
          image,
          keywords: `${tags}, dani blog, blog, , vue, , discord, daniduese`,
          url: href,
        },
        [
          {
  ...
  computed: {
    ...
    getPostImage(): string {
      return this.post?.image
        ? `https://bitstore.dev/${this.post?.image}`
        : `https://bitstore.dev/assets/images/posts/${this.post?.slug}.jpg`
    },
  },
})
</script>
    ...
    <div v-else class="space-x-6 pt-4">
      <div class="mx-auto w-full" :class="post.header && 'space-y-10'">
        ...
        <div class="space-y-12 mt-10 px-4">
          ...
          <!-- Share -->
          <div class="space-y-2">
            <Title :padding="false">Share the article</Title>
...
.nuxt-content {
  ...
  /* Block quotes */
  blockquote {
    @apply rounded-md bg-gray-100 ml-0 w-full py-2 px-4 w-max max-w-full dark:bg-neutral-800;
  }
